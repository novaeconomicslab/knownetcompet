...
class CompaniesGoldSource(BaseDataSource):
    source_name = "companies_gold"
    ...
    def run(self):
        ...
        # 1. Fetch Silver Data
        dm_silver = self.db_connector.get_all_documents("companies_dm_silver")
        self.logger.info(f"Loaded {len(dm_silver)} records from companies_dm_silver.")
        sh_silver = self.db_connector.get_all_documents("companies_sh_silver")
        self.logger.info(f"Loaded {len(sh_silver)} records from companies_sh_silver.")
        ...
        if gold_docs:
            ...
            self.logger.warning("No valid gold records generated for Companies Gold.")
